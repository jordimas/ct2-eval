...
# Parse CLI arguments
# ------------------------
parser = argparse.ArgumentParser(description="WER benchmark across devices and compute types")
parser.add_argument(
    "--audio_numb",
    type=int,
    default=10,
...
for device in devices:
    ...
    for compute_type in sorted(supported_compute_types):
        ...
        # Compute WER
        word_error_rate = 100 * wer(hypothesis=all_transcriptions_norm, reference=all_references_norm)
        ...
        print(f"  Real-time factor: {rtf:.4f} (lower is faster)")
...
print("SUMMARY")
print("=" * 80)
print(f"{'Device':<10} {'Compute Type':<15} {'WER (%)':<12} {'Time (s)':<12} {'RTF':<12} {'Speed':<12}")
print("-" * 80)
for r in results:
    print(f"{r['device']:<10} {r['compute_type']:<15} {r['wer']:<12.3f} {r['time']:<12.2f} {r['rtf']:<12.4f} {r['speed']:<10.2f}x")
