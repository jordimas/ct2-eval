...
def load_translation_data(args):
    ...
    samples = [
        {"src": s, "ref": t, "tokens": tokenizer.encode(s, out_type=str)}
...
MODELS = {
    "whisper": {"model": "small"},
    "translation": {
        "model": "softcatala/translate-eng-cat",
        "tokenizer": "softcatala/translate-eng-cat",
    },
...
def main():
    ...
    parser.add_argument(
        "--mode", required=True, choices=["whisper", "translation", "generation"]
    )
    ...
    print(
        f"Runs: {args.warmup_runs} warmup + {args.num_runs} timed | CTranslate2: {ctranslate2.__version__}"
    )
    print("=" * 90)
        ...
        for ct in compute_types:
            ...
            try:
                if args.mode == "whisper":
                    ...
                    model = WhisperModel(
                        args.model_path, device=device, compute_type=ct
                    )
            ...
            print(
                f"    → {metric_name}={np.mean(metrics):.2f}±{cv_percent(*results[-1]['metric']):.1f}% "
    ...
    print("\n" + "=" * 90)
    print(
        f"{'Device':<8} {'Compute':<12} {metric_name:<20} {'Time (s)':<20} {throughput_name:<20}"
    )
    print("-" * 90)
    for r in results:
        m, t, th = r["metric"], r["time"], r["throughput"]
        print(
            f"{r['device']:<8} {r['compute_type']:<12} "
