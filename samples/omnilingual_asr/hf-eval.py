...
def evaluate_language(
    ...
    print(f"  RTF: {result.avg_rtf:.3f} ({1/result.avg_rtf:.1f}x real-time)" if result.avg_rtf > 0 else "  RTF: N/A")
...
def print_summary(all_results: list[tuple[str, EvalResult]]):
    ...
    print(f"{'='*80}")
    print(f"{'Model':<25} {'Language':<12} {'Samples':<10} {'WER':<10} {'CER':<10} {'RTF':<10}")
    print("-" * 80)
    
    for model_name, r in all_results:
        print(f"{model_name:<25} {r.language:<12} {r.num_samples:<10} {r.wer:<10.2%} {r.cer:<10.2%} {r.avg_rtf:<10.3f}")
...
def main():
    ...
    if args.output:
        ...
        with open(args.output, "w", newline="") as f:
            ...
            for model_name, r in all_results:
                writer.writerow([
                    model_name, r.language, r.num_samples,
                    f"{r.wer:.4f}", f"{r.cer:.4f}", f"{r.avg_rtf:.4f}"
                ])
