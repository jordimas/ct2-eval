...
for compute_type in ["int8", "float32"]:
	...
	translator = ctranslate2.Translator("eng-cat/ctranslate2/", compute_type=compute_type)
	start_time = time.time()

	total_tokens = 0
	for i in range (0, 10):
		# Tokenize each sentence
		tokenized_sentences = [tokenizer.encode(sentence, out_type=str) for sentence in sentences]
		...
		translations = [
		    tokenizer.decode(t[0]["tokens"]) for t in translated_batches
		]
